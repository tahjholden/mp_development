 ...
 */
export async function getPersonRole(personId: string): Promise<PersonType | null> {
 ...
 */
export async function hasRole(personId: string, role: PersonType): Promise<boolean> {
 ...
 */
async function isCoachAssignedToTeam(coachId: string, teamId: string): Promise<boolean> {
  const supabase = createClient();
 ...
 */
async function isParentOfPlayer(parentId: string, playerId: string): Promise<boolean> {
  const supabase = createClient();
 ...
 */
async function isObserverAssignedToTeam(observerId: string, teamId: string): Promise<boolean> {
  const supabase = createClient();
...
export async function getDataAccessConditions(
  ...
  switch (personType) {
    case PersonType.PLAYER:
      // Players can only see their own data
      if (tableName === 'mpbc_development_plan' || tableName === 'mpbc_observations') {
...
export async function getPersonsByRole(
  ...
  const { data, error } = await supabase
    .from('mpbc_person')
    .select('id, person_type, organization_id, is_admin, is_superadmin, display_name')
 ...
 */
export async function getAvailableRoles(organizationId: string): Promise<PersonType[]> {
 ...
 */
export async function getRoleUiConfig(personId: string): Promise<Record<string, any>> {
  const supabase = createClient();
  ...
  if (person.is_superadmin) {
    return {
      ...
      showSettings: true,
      showAdmin: true,
      navigationItems: ['dashboard', 'players', 'teams', 'coaches', 'observations', 'settings', 'admin']
    };
  }
  ...
  if (person.is_admin) {
    ...
    uiConfig.navigationItems = ['dashboard', 'players', 'teams', 'coaches', 'observations', 'settings', 'admin'];
    
    // Adjust based on pack features
    if (!packFeatures.philosophyOverlay) {
      uiConfig.showAdmin = false;
      uiConfig.navigationItems = uiConfig.navigationItems.filter(item => item !== 'admin');
    }
 ...
 */
export async function getPersonRolesWithContext(personId: string): Promise<Array<{role: string, context: RoleContext}>> {
  const supabase = createClient();
...
export async function isFeatureAvailableForRole(
  ...
  const featureAvailable = await isFeatureAvailable(person.organization_id, feature as any);
  ...
  const roleFeatures: Record<PersonType, string[]> = {
    ...
    [PersonType.COACH]: ['playerManagement', 'basicObservations', 'simplePdp', 'aiEnabled', 'drillRecommendations'],
    [PersonType.ADMIN]: ['playerManagement', 'basicObservations', 'simplePdp', 'aiEnabled', 'philosophyOverlay', 'advancedConstraints', 'drillRecommendations'],
    [PersonType.SUPERADMIN]: ['playerManagement', 'basicObservations', 'simplePdp', 'aiEnabled', 'philosophyOverlay', 'advancedConstraints', 'drillRecommendations'],
    [PersonType.PARENT]: ['playerManagement', 'basicObservations'],
    [PersonType.OBSERVER]: ['playerManagement', 'basicObservations', 'aiEnabled']
  };
