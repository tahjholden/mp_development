...
import { Plus, Edit, Eye, Trash2, Star, Tag, ChevronDown, ChevronUp, Search, Filter, TrendingUp, BarChart3, PieChart, Activity, Target, Users, Calendar } from 'lucide-react'
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const validatedMetrics = AnalyticsMetricsArraySchema.safeParse(mockMetrics);
        ...
        const validMetrics = validatedMetrics.data.filter((metric): metric is AnalyticsMetric => 
    ...
    return (
      <div className="h-screen w-screen bg-[#161616] flex items-center justify-center">
        <div className="flex flex-col items-center justify-center w-full">
          <span className="text-zinc-400 text-lg font-semibold mb-4">Loading analytics...</span>
    ...
    <div className="flex min-h-screen h-full bg-black text-white" style={{ background: 'black' }}>
      ...
      <div className="flex-1 flex ml-64 pt-16 bg-black min-h-screen" style={{ background: 'black', minHeight: '100vh' }}>
        ...
        <div className="w-1/4 border-r border-zinc-800 p-6 bg-black flex flex-col justify-start min-h-screen" style={{ background: 'black' }}>
          <h2 className="text-xl font-bold mb-6 text-[#d8cc97] mt-0">Metrics</h2>
          ...
          <div className="relative mb-6">
            ...
            {isCategoryDropdownOpen && (
              <div className="absolute z-10 mt-1 w-full bg-zinc-800 border border-zinc-700 rounded shadow-lg overflow-hidden">
                ...
                {['performance', 'attendance', 'development', 'team'].map(category => (
                  <button
                    key={category}
                    onClick={() => { setCategoryFilter(category); setIsCategoryDropdownOpen(false); }}
          ...
          <div className="space-y-3 max-h-[calc(100vh-400px)] overflow-y-auto">
            {filteredMetricsList.length === 0 ? (
              <div className="text-sm text-gray-500 text-center py-4">No metrics found</div>
            ) : (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-zinc-800 p-6 rounded" style={{ background: '#181818' }}>
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-lg font-bold text-[#d8cc97]">{selectedMetric.name}</h3>
                    <p className="text-sm text-zinc-400 capitalize">{selectedMetric.category} • {selectedMetric.period}</p>
                  </div>
                  <div className="flex gap-2">
                    <button className="text-xs text-[#d8cc97] font-semibold hover:underline bg-transparent" style={{ background: 'transparent' }}>Export</button>
                    <button className="text-xs text-red-400 font-semibold hover:underline bg-transparent" style={{ background: 'transparent' }}>Share</button>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4 text-sm mb-4">
                  <div>
                    <p className="text-zinc-400">Current Value</p>
                    <p className="text-white text-lg font-bold">{selectedMetric.value}%</p>
                  </div>
                  <div>
                    <p className="text-zinc-400">Change</p>
                    <p className={`text-white text-lg font-bold ${selectedMetric.changeType === 'increase' ? 'text-green-400' : selectedMetric.changeType === 'decrease' ? 'text-red-400' : 'text-gray-400'}`}>
                  ...
                  <div>
                    ...
                    <p className={`text-white ${selectedMetric.value >= (selectedMetric.target || 0) ? 'text-green-400' : 'text-yellow-400'}`}>
                      {selectedMetric.value >= (selectedMetric.target || 0) ? 'On Track' : 'Needs Attention'}
                    </p>
                  </div>
                </div>
              ...
              {/* Top Performers */}
              <div className="bg-zinc-800 p-6 rounded" style={{ background: '#181818' }}>
                <h4 className="text-base font-bold text-[#d8cc97] mb-4">Top Performers</h4>
                <div className="space-y-2">
                  {players.sort((a, b) => b.performance - a.performance).slice(0, 5).map((player) => (
                    <div key={player.id} className="flex justify-between items-center p-2 bg-zinc-700 rounded">
                      <span className="text-sm text-white">{player.name}</span>
                      <span className="text-xs text-green-400">{player.performance}%</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="w-1/4 p-8 bg-black flex flex-col justify-start min-h-screen" style={{ background: 'black' }}>
          <h2 className="text-xl font-bold mb-6 text-[#d8cc97] mt-0">Dashboard</h2>
          
          {/* Quick Stats */}
          <div className="bg-zinc-800 p-6 rounded mb-6">
            <h3 className="text-base font-bold text-[#d8cc97] mb-4">Quick Stats</h3>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-zinc-400">Total Players</span>
                <span className="text-sm text-white font-semibold">{players.length}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-zinc-400">Active Teams</span>
                <span className="text-sm text-white font-semibold">{teams.length}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-zinc-400">Avg Performance</span>
                <span className="text-sm text-white font-semibold">
                  {Math.round(players.reduce((sum, p) => sum + p.performance, 0) / players.length)}%
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-zinc-400">Avg Attendance</span>
                <span className="text-sm text-white font-semibold">
                  {Math.round(players.reduce((sum, p) => sum + p.attendance, 0) / players.length)}%
                </span>
              </div>
            </div>
          </div>
          ...
          {/* Team Performance */}
          <div className="bg-zinc-800 p-6 rounded mb-6">
            <h3 className="text-base font-bold text-[#d8cc97] mb-4">Team Performance</h3>
            <div className="space-y-3">
              {teams.sort((a, b) => b.performance - a.performance).slice(0, 3).map((team) => (
                <div key={team.id} className="p-3 bg-zinc-700 rounded">
                  <p className="text-sm font-semibold text-white">{team.name}</p>
                  <p className="text-xs text-zinc-400">{team.performance}% performance • {team.players} players</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-zinc-800 p-6 rounded">
            <h3 className="text-base font-bold text-[#d8cc97] mb-4">Recent Trends</h3>
            <div className="space-y-3">
              {metrics.filter(m => m.changeType === 'increase').slice(0, 3).map((metric) => (
                <div key={metric.id} className="p-3 bg-zinc-700 rounded">
                  <p className="text-sm font-semibold text-white">{metric.name}</p>
                  <p className="text-xs text-green-400">+{metric.change}% improvement</p>
                </div>
              ))}
            </div>
          </div>
