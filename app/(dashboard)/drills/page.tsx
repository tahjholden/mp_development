...
import { Plus, Edit, Eye, Trash2, Star, Tag, ChevronDown, ChevronUp, Search, Filter, Target, Clock, Users, Zap } from 'lucide-react'
...
const DrillSchema = z.object({
  id: z.string(),
  name: z.string(),
  category: z.enum(['shooting', 'ball_handling', 'defense', 'conditioning', 'team_play', 'passing', 'rebounding']),
...
interface Drill {
  id: string
  name: string
  category: 'shooting' | 'ball_handling' | 'defense' | 'conditioning' | 'team_play' | 'passing' | 'rebounding'
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const validDrills = validatedDrills.data.filter((drill): drill is Drill => 
    ...
    return (
      <div className="h-screen w-screen bg-[#161616] flex items-center justify-center">
        <div className="flex flex-col items-center justify-center w-full">
          <span className="text-zinc-400 text-lg font-semibold mb-4">Loading drills...</span>
    ...
    <div className="flex min-h-screen h-full bg-black text-white" style={{ background: 'black' }}>
      ...
      <div className="flex-1 flex ml-64 pt-16 bg-black min-h-screen" style={{ background: 'black', minHeight: '100vh' }}>
        ...
        <div className="w-1/4 border-r border-zinc-800 p-6 bg-black flex flex-col justify-start min-h-screen" style={{ background: 'black' }}>
          ...
          <div className="relative mb-6">
            ...
            {isCategoryDropdownOpen && (
              <div className="absolute z-10 mt-1 w-full bg-zinc-800 border border-zinc-700 rounded shadow-lg overflow-hidden">
                ...
                {['shooting', 'ball_handling', 'defense', 'conditioning', 'team_play', 'passing', 'rebounding'].map(category => (
                  <button
                    key={category}
          ...
          <div className="relative mb-6">
            ...
            >
              ...
              {isDifficultyDropdownOpen ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
            </button>
            {isDifficultyDropdownOpen && (
              <div className="absolute z-10 mt-1 w-full bg-zinc-800 border border-zinc-700 rounded shadow-lg overflow-hidden">
                <button
                  onClick={() => { setDifficultyFilter('all'); setIsDifficultyDropdownOpen(false); }}
                  className="w-full text-left px-4 py-2 text-sm hover:bg-zinc-700 text-white"
                >
                  All Difficulties
                </button>
                {['beginner', 'intermediate', 'advanced'].map(difficulty => (
                  <button
                    key={difficulty}
                    onClick={() => { setDifficultyFilter(difficulty); setIsDifficultyDropdownOpen(false); }}
                    className="w-full text-left px-4 py-2 text-sm hover:bg-zinc-700 text-white capitalize"
                  >
                    {difficulty}
                  </button>
          ...
          <div className="space-y-3 max-h-[calc(100vh-400px)] overflow-y-auto">
            {filteredDrillsList.length === 0 ? (
              <div className="text-sm text-gray-500 text-center py-4">No drills found</div>
            ) : (
              filteredDrillsList.map((drill) => (
                ...
                >
                  ...
                  <p className="text-xs text-gray-400">
                    {drill.category.replace('_', ' ')} • {drill.difficulty} • {drill.duration}min
                  </p>
                </div>
              ))
            )}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-zinc-800 p-6 rounded" style={{ background: '#181818' }}>
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-lg font-bold text-[#d8cc97]">{selectedDrill.name}</h3>
                    <p className="text-sm text-zinc-400 capitalize">{selectedDrill.category.replace('_', ' ')} • {selectedDrill.difficulty}</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm mb-4">
                  <div>
                    ...
                    <p className="text-white">{selectedDrill.duration} minutes</p>
                  </div>
                  <div>
                    <p className="text-zinc-400">Players Needed</p>
                    <p className="text-white">{selectedDrill.players} minimum</p>
                  </div>
                  <div>
                    <p className="text-zinc-400">Equipment</p>
                    <p className="text-white">{selectedDrill.equipment.join(', ')}</p>
                  </div>
                  <div>
                    <p className="text-zinc-400">Category</p>
                    <p className="text-white capitalize">{selectedDrill.category.replace('_', ' ')}</p>
                  </div>
                </div>
                <div className="mb-4">
                  <p className="text-zinc-400 text-sm">Description</p>
                  <p className="text-white text-sm">{selectedDrill.description}</p>
                </div>
              </div>

              {/* Instructions */}
              <div className="bg-zinc-800 p-6 rounded" style={{ background: '#181818' }}>
                <h4 className="text-base font-bold text-[#d8cc97] mb-4">Instructions</h4>
                <ol className="list-decimal list-inside space-y-2">
                  {selectedDrill.instructions.map((instruction, index) => (
                    <li key={index} className="text-sm text-white">{instruction}</li>
                  ))}
                </ol>
              </div>
              ...
              {/* Coaching Cues */}
              <div className="bg-zinc-800 p-6 rounded" style={{ background: '#181818' }}>
                <h4 className="text-base font-bold text-[#d8cc97] mb-4">Coaching Cues</h4>
                <div className="flex flex-wrap gap-2">
                  {selectedDrill.cues.map((cue, index) => (
                    <span key={index} className="px-3 py-1 bg-zinc-700 rounded text-xs text-white">
                      {cue}
                    </span>
                  ))}
                </div>
        ...
        <div className="w-1/4 p-8 bg-black flex flex-col justify-start min-h-screen" style={{ background: 'black' }}>
          <h2 className="text-xl font-bold mb-6 text-[#d8cc97] mt-0">Management</h2>
          ...
          {/* Drill Favorites */}
          <div className="bg-zinc-800 p-6 rounded mb-6">
            <h3 className="text-base font-bold text-[#d8cc97] mb-4">Favorites</h3>
            <div className="space-y-3">
              <div className="p-3 bg-zinc-700 rounded cursor-pointer hover:bg-zinc-600 transition-colors">
                <p className="text-sm font-semibold text-white">3-Point Shooting Progression</p>
                <p className="text-xs text-zinc-400">20 min • Intermediate</p>
              </div>
              <div className="p-3 bg-zinc-700 rounded cursor-pointer hover:bg-zinc-600 transition-colors">
                <p className="text-sm font-semibold text-white">Ball Handling Circuit</p>
          ...
          <div className="bg-zinc-800 p-6 rounded">
            <h3 className="text-base font-bold text-[#d8cc97] mb-4">Related Drills</h3>
            <div className="space-y-3">
              {drills.filter(d => d.category === selectedDrill?.category && d.id !== selectedDrill?.id).slice(0, 3).map((drill) => (
                <div key={drill.id} className="p-3 bg-zinc-700 rounded cursor-pointer hover:bg-zinc-600 transition-colors">
                  <p className="text-sm font-semibold text-white">{drill.name}</p>
                  <p className="text-xs text-zinc-400">{drill.duration} min • {drill.difficulty}</p>
                </div>
              ))}
            </div>
          </div>
