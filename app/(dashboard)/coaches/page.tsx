...
import { Plus, Edit, Eye, Trash2, Star, Tag, ChevronDown, ChevronUp, Search, Filter, Users, Award, Calendar } from 'lucide-react'
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const validCoaches = validatedCoaches.data.filter((coach): coach is Coach => 
    ...
    <div className="flex min-h-screen h-full bg-black text-white" style={{ background: 'black' }}>
      ...
      <div className="flex-1 flex ml-64 pt-16 bg-black min-h-screen" style={{ background: 'black', minHeight: '100vh' }}>
        ...
        <div className="w-1/4 border-r border-zinc-800 p-6 bg-black flex flex-col justify-start min-h-screen" style={{ background: 'black' }}>
          <h2 className="text-xl font-bold mb-6 text-[#d8cc97] mt-0">Coaches</h2>
          ...
          <div className="space-y-3 max-h-[calc(100vh-400px)] overflow-y-auto">
            {filteredCoachesList.length === 0 ? (
              <div className="text-sm text-gray-500 text-center py-4">No coaches found</div>
            ) : (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-zinc-800 p-6 rounded" style={{ background: '#181818' }}>
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-lg font-bold text-[#d8cc97]">{selectedCoach.name}</h3>
                    <p className="text-sm text-zinc-400">{selectedCoach.role}</p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button className="text-xs text-red-400 font-semibold hover:underline bg-transparent" style={{ background: 'transparent' }}>Remove</button>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-white">{selectedCoach.experience} years</p>
                  </div>
                  <div>
                    <p className="text-zinc-400">Status</p>
                    <p className={`text-white ${selectedCoach.status === 'active' ? 'text-green-400' : 'text-red-400'}`}>
              ...
              {/* Assigned Players */}
              <div className="bg-zinc-800 p-6 rounded" style={{ background: '#181818' }}>
                <h4 className="text-base font-bold text-[#d8cc97] mb-4">Assigned Players ({selectedCoach.playersCount})</h4>
                <div className="space-y-2">
                  {players.filter(p => p.team === selectedCoach.team).slice(0, 5).map((player) => (
                    <div key={player.id} className="flex justify-between items-center p-2 bg-zinc-700 rounded">
                      <span className="text-sm text-white">{player.name}</span>
                      <span className="text-xs text-zinc-400">{player.status}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="w-1/4 p-8 bg-black flex flex-col justify-start min-h-screen" style={{ background: 'black' }}>
          ...
          <div className="bg-zinc-800 p-6 rounded mb-6">
            <h3 className="text-base font-bold text-[#d8cc97] mb-4">Performance Metrics</h3>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-zinc-400">Total Players</span>
                <span className="text-sm text-white font-semibold">{coaches.reduce((sum, coach) => sum + coach.playersCount, 0)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-zinc-400">Active Coaches</span>
                <span className="text-sm text-white font-semibold">{coaches.filter(c => c.status === 'active').length}</span>
              </div>
              <div className="flex justify-between items-center">
